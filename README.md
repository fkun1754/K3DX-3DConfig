...
> - App 检测到 SELinux 为 Enforcing 时，开启游戏深度悬浮窗会提示需要 root
> - **已获取 root 时 App 自动执行 `setenforce 0`**，无需手动操作
> - 未 root 且 Enforcing 的设备无法深度调节
...
- **1.2.2**：美化UI，文字说明；修复屏幕旋转的逻辑
- **1.2.1**：修复V5G补丁不生效的bug；优化了一些UI和说明
- **1.2**：新增应用选择页面和模板选择页面搜索功能（模板推荐置顶）；3D应用悬浮窗模式/视角按钮独立开关；修复V5G横屏旋转视角不跟随的bug（180度旋转自动切换视角）；优化UI与性能
