...
- **3D games**: rely on the system libGLES injection layer reading `.gles.cfg` — the config token carries a vendor-proprietary checksum and cannot be generated by users
...
| **Changhong X1** | ❌ | ❌ | ❌ | Not working (different system mechanism) |
...
### 🪟 Floating Window (dual-mode)
- Auto-switches between game depth slider / 3D-app dual buttons (based on foreground app)
- Edge-collapse, position memory, only shown for configured apps
- Notification quick-toggle button

### 🔒 Process Guard (optional, root)
- Magisk module: boot autostart + auto-restart if killed + OOM protection
- nubia system process whitelist injection (background persistence)
...
|------|------|-----------|
| `WRITE/READ_EXTERNAL_STORAGE` | Read/write `/sdcard/.gles.cfg` | 6.0+ runtime |
...
- Depth: `setprop persist.sys.3deffect (0-20)` — **no root needed** on some devices (e.g. K3DX-V5G: SELinux=Permissive, vendor relaxed `persist.*` write permission)
...
**Android support**: minSdk 19 (Android 4.4) to latest — upward adapted (6.0 runtime permissions / 8.0 notification channels / 9.0 foreground service / 11.0 scoped storage / 13.0 notification permission)
